...
<div class="mb-12 text-center md:text-left">
    ...
    <h2 class="text-4xl lg:text-5xl font-black text-slate-800 mb-4 tracking-tight">
        Selamat Datang <br> <span class="text-primary italic">Kembali.</span>
    </h2>
    <p class="text-lg font-bold text-slate-500 max-w-sm">
...
<form method="POST" action="{{ route('login') }}" class="space-y-6">
    ...
    {{-- ── Email Input Field ── --}}
    <div class="group">
        <label for="email" class="ml-4 font-black text-slate-600 uppercase tracking-[0.2em] text-[11px] flex items-center gap-2 mb-2 group-focus-within:text-primary transition-colors">
        ...
        <div class="relative">
            <input id="email" type="email" name="email" value="{{ old('email') }}" required autofocus
                   ...
                   class="w-full h-16 px-6 rounded-3xl bg-surface-container-low border-2 border-transparent text-lg font-bold text-slate-800 placeholder:text-slate-400 focus:outline-none focus:bg-white focus:border-primary/20 focus:ring-4 focus:ring-primary/5 transition-all shadow-sm hover:bg-slate-50">
            <div class="absolute inset-y-0 right-6 flex items-center text-slate-400 group-focus-within:text-primary transition-colors pointer-events-none">
    ...
    <div class="group">
        <div class="flex justify-between items-center px-4 mb-2">
            <label for="password" class="font-black text-slate-600 uppercase tracking-[0.2em] text-[11px] flex items-center gap-2 group-focus-within:text-primary transition-colors">
        ...
        <div class="relative">
            <input id="password" type="password" name="password" required
                   placeholder="••••••••"
                   class="w-full h-16 px-6 pr-16 rounded-3xl bg-surface-container-low border-2 border-transparent text-xl font-black text-slate-800 placeholder:text-slate-300 focus:outline-none focus:bg-white focus:border-primary/20 focus:ring-4 focus:ring-primary/5 transition-all shadow-sm hover:bg-slate-50 tracking-widest">
            
            <button type="button" onclick="togglePassword()" id="toggleBtn"
                    class="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 flex items-center justify-center rounded-xl text-slate-400 hover:bg-white hover:text-primary hover:shadow transition-all active:scale-95">
    ...
    <div class="flex items-center justify-between px-4 pt-2">
        <label class="relative flex items-center gap-3 cursor-pointer group select-none">
            ...
            <span class="text-sm font-bold text-slate-600 group-hover:text-primary transition-colors">Ingat Perangkat</span>
        </label>
    ...
    <div class="pt-6">
        <button type="submit" id="submitBtn"
                class="w-full h-16 bg-primary hover:bg-primary/90 text-white text-lg font-black uppercase tracking-[0.2em] rounded-3xl shadow-lg shadow-primary/30 hover:shadow-xl hover:shadow-primary/40 hover:-translate-y-0.5 active:translate-y-0 active:scale-[0.98] transition-all flex items-center justify-center gap-4 group">
            <span>Masuk</span>
