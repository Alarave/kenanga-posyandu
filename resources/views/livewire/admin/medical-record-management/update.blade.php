    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div class="flex items-center gap-4">
            <div class="w-14 h-14 rounded-2xl bg-primary/10 flex items-center justify-center border border-primary/20 shadow-sm">
                <span class="material-symbols-outlined text-primary text-display-sm">edit_note</span>
            </div>
            <div>
                <h1 class="text-headline-md font-black text-on-surface tracking-tight">Edit Pemeriksaan</h1>
                <p class="text-sm font-medium text-outline">Memperbarui data kesehatan balita</p>
            </div>
        </div>
        
        <div class="flex items-center gap-3">
            <div class="px-4 py-2 rounded-2xl bg-white border border-outline-variant shadow-sm flex items-center gap-2">
                <span class="w-2 h-2 rounded-lg bg-primary animate-pulse"></span>
                <span class="text-xs font-bold text-on-surface-variant uppercase tracking-wider">ID REKAM: #{{ $record->id }}</span>
            </div>
        </div>
    </div>
            ...
            <div class="bg-white/70 backdrop-blur-xl rounded-[3rem] border border-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-10 relative overflow-hidden group transition-all duration-500 hover:shadow-[0_20px_50px_rgba(0,108,73,0.1)]">
                ...
                <div class="grid grid-cols-1 md:grid-cols-12 gap-8 relative">
                    ...
                    <div class="md:col-span-8 space-y-3">
                        <label class="text-[11px] font-black text-outline-variant uppercase tracking-widest ml-1">Nama Balita / Sasaran <span class="text-primary">*</span></label>
                        ...
                        <p class="text-[10px] text-outline-variant italic ml-1">Nama sasaran tidak dapat diubah setelah rekam dibuat.</p>
...
])>{{ old('current_medication', $record->current_medication) }}</textarea>
                        ...
                        <div class="space-y-6 relative">
                            <div class="grid grid-cols-1 gap-3">
                                @foreach([
                                    ...
                                    <label class="relative flex items-center justify-between p-5 rounded-2xl border border-slate-100 bg-surface-container-low/30 hover:bg-white hover:border-primary hover:shadow-md transition-all cursor-pointer group/item has-checked:border-teal-600 has-checked:bg-teal-400 shadow-sm">
                                        <div class="flex items-center gap-4">
                                            <div class="w-10 h-10 rounded-xl bg-white border border-slate-100 flex items-center justify-center text-outline-variant group-hover/item:text-primary transition-colors shadow-sm">
                                                <span class="material-symbols-outlined text-[20px] group-has-checked:text-primary">check_circle</span>
                                            </div>
                                            <span class="text-sm font-bold text-on-surface-variant select-none group-has-checked:text-black">{{ $label }}</span>
                                        </div>
                                        <input type="checkbox" name="{{ $name }}" value="1" {{ old($name, $record->$name) ? 'checked' : '' }}
                                               class="w-6 h-6 rounded-lg border-outline-variant text-primary focus:ring-primary/20 transition-all accent-teal-600">
                                    </label>
                                @endforeach
                            </div>
                            ...
                            <div class="space-y-3 pt-2">
                                ...
                                <textarea name="other_symptoms" rows="10" placeholder="Sebutkan gejala lain jika ada secara detail..."
                                          class="w-full p-6 border border-outline-variant rounded-2xl text-sm font-bold text-on-surface-variant focus:outline-none focus:border-primary focus:ring-4 focus:ring-primary/5 transition-all bg-surface-container-low/30 resize-none">{{ old('other_symptoms', $record->other_symptoms) }}</textarea>
                            </div>
                        </div>
                    </div>
                             ...
                             <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                @foreach([
                                    ...
                                    <div x-data="{ checked: {{ old($name, $record->$name) ? 'true' : 'false' }} }" class="space-y-3">
                                        <label class="relative flex flex-col items-center justify-center p-5 rounded-2xl border border-slate-100 bg-surface-container-low/30 hover:bg-white hover:border-primary transition-all cursor-pointer group/toggle has-checked:border-primary/50 has-checked:bg-primary-container/30">
                                            ...
                                            <div class="mt-3 px-3 py-1 rounded-lg text-[9px] font-black tracking-tighter transition-all bg-surface-container text-on-surface peer-checked:bg-teal-100 peer-checked:text-on-surface uppercase" 
                                    ...
                                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                        @foreach(['HB-0', 'Polio 0', 'BCG', 'Polio 1', 'Polio 2', 'Polio 3', 'Polio 4', 'DPT-HB-Hib 1', 'DPT-HB-Hib 2', 'DPT-HB-Hib 3', 'PCV 1', 'PCV 2', 'PCV 3', 'RV 1', 'RV 2', 'RV 3', 'IPV 1', 'IPV 2', 'MR', 'DPT-HB-Hib Lanjutan', 'MR Lanjutan', 'JE'] as $vaccine)
                                            <button type="button" 
                                                    ...
                                                    :class="selectedVaccines.includes('{{ $vaccine }}') ? 'border-teal-600 bg-teal-400 text-black shadow-sm' : 'border-slate-100 bg-surface-container-low/30 text-on-surface-variant hover:bg-white hover:border-primary'"
                                                ...
                                                <div class="w-6 h-6 rounded-lg flex items-center justify-center transition-colors"
                                                     :class="selectedVaccines.includes('{{ $vaccine }}') ? 'bg-white text-primary' : 'bg-white border border-slate-100 text-slate-300 group-hover/vax:text-teal-500'">
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="space-y-3">
                            ...
                            <textarea name="complaint" rows="2" placeholder="Catat keluhan balita jika ada..."
                                      class="w-full p-5 border border-outline-variant rounded-2xl text-sm font-bold text-on-surface-variant focus:outline-none focus:border-primary focus:ring-4 focus:ring-primary/5 transition-all bg-surface-container-low/30 resize-none">{{ old('complaint', $record->complaint) }}</textarea>
                        </div>
                        <div class="space-y-3">
                        ...
                        <div class="space-y-3">
                            ...
                            <textarea name="counseling_notes" rows="2" placeholder="Catat poin konseling yang diberikan..."
                                      class="w-full p-5 border border-outline-variant rounded-2xl text-sm font-bold text-on-surface-variant focus:outline-none focus:border-primary focus:ring-4 focus:ring-primary/5 transition-all bg-surface-container-low/30 resize-none">{{ old('counseling_notes', $record->counseling_notes) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
