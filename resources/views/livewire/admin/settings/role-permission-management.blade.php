<div class="space-y-6">
    ...
    @if (session()->has('message'))
        <div class="p-4 mb-4 text-sm text-on-primary-container bg-primary-container rounded-2xl border border-teal-100 flex items-center gap-3">
            ...
            <table class="w-full">
                <thead>
                    <tr class="bg-surface-container-low/50 border-b border-slate-100">
                        <th class="px-8 py-5 text-left text-[10px] font-black text-on-surface uppercase tracking-widest min-w-[250px]">Fitur / Izin</th>
                        @foreach($roles as $role)
                            <th class="px-8 py-5 text-center text-[10px] font-black text-on-surface uppercase tracking-widest">{{ $role->display_name }}</th>
                        @endforeach
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-50">
                    @foreach($permissions as $permission)
                    <tr class="group hover:bg-primary-container/30 transition-all duration-300">
                        <td class="px-8 py-6">
                            <div class="flex flex-col">
                                <span class="font-black text-on-surface">{{ $permission->description }}</span>
                                <span class="text-[10px] font-black text-outline-variant uppercase tracking-widest mt-1">{{ $permission->name }}</span>
                            </div>
                        </td>
                        @foreach($roles as $role)
                        <td class="px-8 py-6 text-center">
                            <label class="relative inline-flex items-center cursor-pointer group/toggle">
                                ...
                                <div class="w-11 h-6 bg-surface-container-high peer-focus:outline-none rounded-lg peer peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-lg after:h-5 after:w-5 after:transition-all peer-checked:bg-primary"></div>
                            </label>
                        </td>
                        @endforeach
                    </tr>
    ...
    {{-- ── Info Card ── --}}
    <div class="bg-inverse-surface rounded-[2.5rem] p-8 text-white relative overflow-hidden">
        <div class="absolute top-0 right-0 p-8 opacity-10">
            <span class="material-symbols-outlined text-[120px]">security</span>
        </div>
        <div class="relative z-10 max-w-2xl">
            <h3 class="text-headline-sm font-black mb-4">Informasi Keamanan</h3>
            <p class="text-outline-variant text-sm leading-relaxed mb-6 font-medium">
            ...
            <div class="flex items-center gap-4">
                <div class="flex items-center gap-2 px-4 py-2 bg-white/5 rounded-xl border border-white/10">
                    <div class="w-2 h-2 rounded-lg bg-teal-400 animate-pulse"></div>
