...
<div class="max-w-7xl mx-auto space-y-6 pb-20 px-4 sm:px-6">
    ...
    <section class="relative rounded-2xl overflow-hidden" style="background:#0f172a;">
        ...
        <div
            ...
            <div class="flex items-center no-print mt-4 md:mt-0">
                <div
                    class="text-xs md:text-sm font-bold text-teal-100 bg-white/10 border border-white/20 backdrop-blur-xs px-4 py-2.5 rounded-xl shadow-xs">
    ...
    <div
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-end pb-6 border-b border-outline-variant/20">
            <div
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            <select wire:model.live="selectedPosyandu"
                                class="w-full bg-surface-container-low/50 border border-outline-variant/30 rounded-xl text-body-md py-2.5 pl-4 pr-10 focus:ring-2 focus:ring-primary/20 transition-all appearance-none cursor-pointer">
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        <select wire:model.live="selectedMonth" @if ($activeTab === 'overview' && $viewMode === 'yearly') disabled @endif
                            class="w-full bg-surface-container-low/50 border border-outline-variant/30 rounded-xl text-body-md py-2.5 pl-4 pr-10 focus:ring-2 focus:ring-primary/20 transition-all appearance-none disabled:opacity-50 disabled:cursor-not-allowed {{ $activeTab === 'overview' && $viewMode === 'yearly' ? 'cursor-not-allowed' : 'cursor-pointer' }}">
                            <option value="">
                                {{ $activeTab === 'overview' && $viewMode === 'yearly' ? 'Nonaktif (Mode YoY)' : 'Semua Bulan (Tahunan)' }}
                            </option>
                            @foreach (range(1, 12) as $m)
                                <option value="{{ $m }}">
                                    {{ Carbon\Carbon::create(2000, $m)->translatedFormat('F') }}</option>
                            @endforeach
                        </select>
                        <span
                            class="material-symbols-outlined absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-outline">expand_more</span>
        ...
        {{-- Bottom bar of control card --}}
        <div class="flex flex-wrap items-center justify-between pt-6 gap-4">
                ...
                @php
                    $overviewCards = [
                        [
                            ...
                            'val' => number_format($totalKunjungan),
                            'unit' => 'Pemeriksaan',
                            'icon' => 'analytics',
                            'color' => 'slate',
                            'desc' => 'Total rekam medis terdaftar',
                        ],
                        [
                            'label' => 'Balita & Anak',
                            'val' => number_format($totalBalita),
                            'unit' => 'Jiwa',
                            'icon' => 'child_care',
                            'color' => 'teal',
                            'desc' => 'Kategori Balita, Bayi & Baduta',
                        ],
                        [
                            'label' => 'Ibu Hamil',
                            'val' => number_format($totalIbuHamil),
                            'unit' => 'Jiwa',
                            'icon' => 'pregnant_woman',
                            'color' => 'rose',
                            'desc' => 'Ibu mengandung terdaftar',
                        ],
                        [
                            'label' => 'Lansia',
                            'val' => number_format($totalLansia),
                            'unit' => 'Jiwa',
                            'icon' => 'elderly',
                            'color' => 'indigo',
                            'desc' => 'Kategori Lanjut Usia',
                        ],
                    ];
                @endphp
                ...
                @php
                    $balitaCards = [
                        [
                            ...
                            'icon' => 'child_care',
                            'color' => 'teal',
                            'desc' => 'Tumbuh kembang aktif terpantau',
                        ],
                        [
                            ...
                            'icon' => 'trending_down',
                            'color' => 'red',
                            'desc' => $stuntingRate >= 14 ? 'Butuh perhatian khusus' : 'Kategori aman & terkendali',
                        ],
                        [
                            'label' => 'Cakupan Imunisasi',
                            'val' => $cakupanImunisasi . '%',
                            'unit' => 'Target 100%',
                            'icon' => 'vaccines',
                            'color' => 'blue',
                            'desc' => 'Persentase dosis imunisasi dasar',
                        ],
                        [
                            'label' => 'Kader Lapangan',
                            'val' => $kaderAktif,
                            'unit' => 'Personel',
                            'icon' => 'badge',
                            'color' => 'amber',
                            'desc' => 'Kader aktif membina balita',
                        ],
                    ];
                @endphp
                    ...
                    <div
                        ...
                        <div class="relative z-10 space-y-3">
                            ...
                            <p class="text-xs md:text-sm text-slate-300 leading-relaxed font-semibold">
                                @if ($stuntingRate >= 14)
                                    Lampu Kuning: Prevalensi stunting mencapai {{ $stuntingRate }}% (ambang batas WHO:
                                    14%). Tingkatkan pemantauan status gizi (tinggi badan & berat badan) dan distribusi
                                    PMT kaya protein.
                                @else
                                    Kondisi Kondusif: Prevalensi stunting terpantau di angka {{ $stuntingRate }}% yang
                                    aman di bawah ambang batas WHO. Pertahankan cakupan pemberian ASI eksklusif dan
                                    pendampingan kader.
                                @endif
                            </p>
                        </div>
...
@script
    <script>
        ...
        function initCharts(data = null) {
            ...
            try {
                if (nutritionTrendChart) {
                    nutritionTrendChart.destroy();
                    nutritionTrendChart = null;
                }
            } catch (e) {}
            try {
                if (nutritionDonutChart) {
                    nutritionDonutChart.destroy();
                    nutritionDonutChart = null;
                }
            } catch (e) {}
            try {
                if (vaccineChart) {
            ...
            try {
                if (pregnancyRiskChart) {
                    pregnancyRiskChart.destroy();
                    pregnancyRiskChart = null;
                }
            } catch (e) {}
            try {
                if (lansiaMetabolicChart) {
                    lansiaMetabolicChart.destroy();
                    lansiaMetabolicChart = null;
                }
            } catch (e) {}
                        ...
                        options: {
                            ...
                            onClick: (event, activeElements) => {
                                if (activeElements.length > 0) {
                                    ...
                                    let type = 'balita';
                                    let dsLabel = '';
                                    if (viewMode === 'yearly' || (compareMode && trendLabelsPrevious.length >
                                        0)) {
                                        if (viewMode === 'yearly') {
                                            ...
                                            dsLabel = catName;
                                            type = catName === 'Balita' ? 'balita' : (catName === 'Ibu Hamil' ?
                                                'ibu_hamil' : 'lansia');
                                        }
                                    } else {
            ...
            if (donutCtx && nutData && nutData.length > 0 && nutData.some(v => v > 0)) {
                try {
                    const colors = nutLabels.map(label => {
                        ...
                        if (label.includes('Sangat') || label.includes('Buruk') || label.includes('Pendek'))
                        return '#ef4444';
                        ...
                        options: {
                            ...
                            onClick: (event, activeElements) => {
                                if (activeElements.length > 0) {
                                    ...
                                    let type = 'balita';
                                    if (label.includes('Buruk') || label.includes('Sangat Kurang')) type =
                                        'gizi_buruk';
                                    else if (label.includes('Kurang') || label.includes('Pendek') || label
                                        .includes('Stunting')) type = 'stunting';
            ...
            if (pregCtx) {
                try {
                    const pregCtx2d = pregCtx.getContext('2d');
                    const gradFe = pregCtx2d.createLinearGradient(0, 0, 0, 300);
                    gradFe.addColorStop(0, 'rgba(16, 185, 129, 0.3)');
                    gradFe.addColorStop(1, 'rgba(16, 185, 129, 0.00)');

                    const gradHyper = pregCtx2d.createLinearGradient(0, 0, 0, 300);
                    gradHyper.addColorStop(0, 'rgba(244, 63, 94, 0.3)');
                    gradHyper.addColorStop(1, 'rgba(244, 63, 94, 0.00)');
