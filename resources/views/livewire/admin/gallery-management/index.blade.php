<div class="space-y-8 p-6 md:p-8 pt-6 sm:pt-8">
    ...
    <div class="bg-white rounded-[2rem] border border-slate-100 p-6 flex flex-col lg:flex-row gap-6 items-center justify-between shadow-[0_8px_30px_rgb(0,0,0,0.015)]">
        <div class="relative w-full group">
            ...
            <input type="text" wire:model.live.debounce.300ms="search" placeholder="Cari nama folder kegiatan..." 
    ...
    @else
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 pt-6">
            @foreach($folders as $folder)
                <div class="relative bg-white rounded-[2rem] border border-slate-100 group hover:-translate-y-1.5 hover:border-teal-200 hover:shadow-[0_20px_40px_rgba(13,148,136,0.05)] transition-all duration-300 flex flex-col h-full mt-4">
                    
                    {{-- Folder Cover / Preview --}}
                    <div class="aspect-[4/3] relative overflow-hidden bg-slate-100 flex items-center justify-center rounded-[1.5rem] m-3 mb-0 border border-slate-200/30 z-20">
                        ...
                        @else
                            ...
                            <div class="absolute inset-0 bg-gradient-to-tr from-teal-50 to-emerald-50/20"></div>
                            <div class="flex flex-col items-center justify-center text-teal-650/80 group-hover:scale-105 transition-transform duration-500 relative z-10">
                                <span class="material-symbols-outlined text-[64px] text-teal-600/30" style="font-variation-settings: 'FILL' 1; font-weight:200;">folder</span>
                                <span class="text-[9px] font-black uppercase tracking-widest text-teal-600/30 mt-1">Kosong</span>
                            </div>
                        @endif
                    ...
                    <div class="p-6 pt-4 flex-1 flex flex-col justify-between space-y-4 z-20">
                        <div class="space-y-2">
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 bg-emerald-50/80 text-emerald-700 rounded-full text-[9px] font-black uppercase tracking-widest border border-emerald-100/50">
                                <span class="w-1.5 h-1.5 rounded-full bg-emerald-400"></span>
                                {{ $folder->posyandu->name ?? 'Semua Posyandu' }}
                            </span>
                            <a href="{{ route('admin.gallery.show', $folder->id) }}" class="block">
                                <h3 class="text-base font-black text-slate-800 leading-snug line-clamp-1 hover:text-teal-650 transition-colors" title="{{ $folder->name }}">
                                    {{ $folder->name }}
                                </h3>
                            </a>
                            <p class="text-xs font-semibold text-slate-450 line-clamp-2 leading-relaxed">
                                {{ $folder->description ?? 'Tidak ada deskripsi folder.' }}
                            </p>
                        </div>
                        <div class="text-[10px] text-slate-400 font-bold flex items-center justify-between pt-4 border-t border-slate-100 flex-wrap gap-2">
                            ...
                            <a href="{{ route('admin.gallery.show', $folder->id) }}" class="text-teal-600 hover:text-teal-700 flex items-center gap-0.5 hover:gap-1.5 font-black uppercase tracking-widest text-[9px] ml-auto lg:ml-0 transition-all duration-200">
                                Buka
                                <span class="material-symbols-outlined text-[13px] transition-transform">arrow_forward</span>
                            </a>
                        </div>
                    </div>
                </div>
