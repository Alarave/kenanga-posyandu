...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-headline-sm font-bold text-on-surface">Edit Data Pedukuhan</h1>
            <p class="text-sm text-outline mt-0.5">Perbarui informasi wilayah pedukuhan</p>
        </div>
    ...
    <div class="bg-white border border-outline-variant rounded-2xl shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-outline-variant flex items-center gap-3">
            ...
            <div>
                <h2 class="text-sm font-bold text-on-surface">{{ $pedukuhan->name }}</h2>
                <p class="text-xs text-outline-variant">Edit informasi pedukuhan</p>
            </div>
        </div>
...
])>{{ old('geo_location', $pedukuhan->geo_location) }}</textarea>
                </div>
                @error('geo_location')
                    <p class="mt-1 text-xs text-error flex items-center gap-1">
                        <span class="material-symbols-outlined text-[13px]">error</span>{{ $message }}
