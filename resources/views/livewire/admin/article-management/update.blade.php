...
<div class="max-w-[720px] mx-auto px-5 md:px-0 pt-12 pb-40">
            ...
            <div class="relative group/row"
                ...
                <div x-show="block.type === 'paragraph'" class="flex items-start gap-2">
                    ...
                    <div :id="'block-' + block.id" contenteditable="true"
                         ...
                         class="flex-1 min-h-[1.8em] py-0.5 outline-none text-[1.15rem] leading-[1.9] text-on-surface-variant ce-placeholder"
                ...
                <div x-show="block.type === 'callout'" class="flex items-start gap-2 my-2">
                    ...
                    <div class="flex-1 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 flex gap-3">
                        <span class="text-headline-sm flex-shrink-0 mt-0.5">💡</span>
                        <div :id="'block-' + block.id" contenteditable="true"
                             x-init="$el.innerHTML = block.content || ''; $el.addEventListener('input', () => { block.content = $el.innerHTML; isDirty = true; });"
                             @keydown="handleKeydown($event, index)" @focus="focusedIndex = index; activeBlockId = block.id" @blur="handleBlur(index)"
                ...
                <div x-show="block.type === 'bullet'" class="flex items-start gap-2">
                    ...
                    <div class="flex-1 flex items-start gap-2 py-0.5">
                        <span class="self-center w-1.5 h-1.5 rounded-lg bg-slate-700 flex-shrink-0"></span>
                        <div :id="'block-' + block.id" contenteditable="true"
                             x-init="$el.innerHTML = block.content || ''; $el.addEventListener('input', () => { block.content = $el.innerHTML; isDirty = true; });"
                             @keydown="handleKeydown($event, index)" @focus="focusedIndex = index; activeBlockId = block.id" @blur="handleBlur(index)"
                ...
                <div x-show="block.type === 'numbered'" class="flex items-start gap-2">
                    ...
                    <div class="flex-1 flex items-start gap-2 py-0.5">
                        <span class="mt-[0.45rem] text-sm font-bold text-outline flex-shrink-0 w-5 text-right" x-text="getNumberedIndex(index) + '.'"></span>
                        <div :id="'block-' + block.id" contenteditable="true"
                             x-init="$el.innerHTML = block.content || ''; $el.addEventListener('input', () => { block.content = $el.innerHTML; isDirty = true; });"
                             @keydown="handleKeydown($event, index)" @focus="focusedIndex = index; activeBlockId = block.id" @blur="handleBlur(index)"
                ...
                <div x-show="block.type === 'video'" class="my-4 ml-9 relative group/vid"
                     ...
                     @keydown.backspace.prevent="removeBlock(index)"
                     @focus="focusedIndex = index">
                    <div x-show="!block.embedSrc && !block.localSrc" class="rounded-2xl bg-surface-container-low border-2 border-dashed border-outline-variant p-6">
                        <p class="text-xs font-bold text-outline uppercase tracking-widest mb-3 text-center">YouTube, Google Drive, atau Upload Video</p>
                        <div class="flex gap-2 mb-3">
                            <input type="text" x-model="block.url"
                                   placeholder="https://youtube.com/watch?v=…"
                                   @keydown.enter.prevent="embedVideo(block)"
                                   class="flex-1 h-10 px-3 rounded-lg border border-outline-variant text-sm text-on-surface-variant focus:outline-none focus:border-indigo-400 bg-white">
                            <button type="button" @click="embedVideo(block)"
                                    class="h-10 px-4 bg-inverse-surface text-white rounded-lg text-label-sm tracking-wide hover:bg-secondary transition-colors">
                                Embed
                            </button>
                        </div>
                        ...
                        <button type="button"
                                ...
                                class="w-full h-10 border-2 border-dashed border-outline-variant hover:border-indigo-400 hover:bg-secondary-container rounded-xl text-xs font-bold text-outline hover:text-secondary transition-all flex items-center justify-center gap-2">
    ...
    <div class="border-t-2 border-dashed border-outline-variant pt-8 space-y-5">
        ...
        <button type="button" @click="submitToLivewire()" :disabled="isSaving"
                class="w-full h-14 bg-secondary hover:bg-indigo-700 text-white rounded-xl text-sm font-black uppercase tracking-widest shadow-lg transition-all active:scale-[0.99] disabled:opacity-60 flex items-center justify-center gap-3 mt-2">
