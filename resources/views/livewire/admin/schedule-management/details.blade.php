...
<div class="space-y-8 p-6 md:p-10">
            ...
            <div class="bg-white rounded-[2.5rem] border border-slate-100 p-8 md:p-10 shadow-sm relative overflow-hidden">
                <div class="relative z-10">
                    <div class="flex items-center gap-4 mb-8">
                        ...
                        @endphp
                        <span class="px-5 py-2 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] border {{ $colors[$schedule->status] ?? 'bg-slate-50' }}">
                            {{ $schedule->status }}
                        </span>
                        <span class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">{{ $schedule->posyandu->name ?? 'Semua Unit' }}</span>
                    </div>

                    <h1 class="text-3xl md:text-5xl font-black text-slate-900 leading-tight mb-6 tracking-tight">{{ $schedule->title }}</h1>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-8 pt-10 border-t border-slate-50">
                        <div class="space-y-6">
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Tanggal Kegiatan</p>
                                    <p class="text-base font-black text-slate-900">{{ \Carbon\Carbon::parse($schedule->start_time)->translatedFormat('d F Y') }}</p>
                                </div>
                            </div>
                            <div class="flex items-center gap-4">
                                <div class="w-12 h-12 rounded-2xl bg-slate-50 text-slate-400 flex items-center justify-center border border-slate-100">
                        ...
                        <div class="space-y-6">
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Lokasi Agenda</p>
                                    <p class="text-base font-black text-slate-900">{{ $schedule->location ?? 'Posyandu Unit' }}</p>
                                </div>
                            </div>
                            <div class="flex items-center gap-4">
                                <div class="w-12 h-12 rounded-2xl bg-slate-50 text-slate-400 flex items-center justify-center border border-slate-100">
                                ...
                                <div>
                                    ...
                                    <p class="text-base font-black text-slate-900">{{ $schedule->user->name ?? 'Sistem' }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="lg:col-span-4 space-y-6">
            ...
            <div class="bg-slate-900 rounded-[2rem] p-8 text-white shadow-xl shadow-slate-200 relative overflow-hidden group">
                <div class="relative z-10">
                    ...
                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Total Pasien Hadir</p>
                    ...
                    <p class="text-[10px] font-bold text-slate-500 mt-4 leading-relaxed">Persentase kehadiran berdasarkan rata-rata kunjungan bulanan posyandu.</p>
                </div>
            </div>
            ...
            <div class="bg-white rounded-[2rem] border border-slate-100 p-8 shadow-sm">
                ...
                <h3 class="text-5xl font-black text-slate-900 tracking-tighter mb-6">{{ $schedule->medical_records_count ?? 0 }}</h3>
    ...
    <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
        <div class="px-10 py-8 border-b border-slate-50 flex items-center justify-between bg-slate-50/20">
            ...
            <span class="px-4 py-1.5 bg-slate-100 text-slate-400 rounded-xl text-[10px] font-black uppercase tracking-widest">Live Updates</span>
        </div>
        ...
        <div class="overflow-x-auto">
            ...
            <table class="w-full">
                <thead class="bg-slate-50/50">
                    <tr>
                        <th class="px-10 py-5 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">Identitas Pasien</th>
                        <th class="px-10 py-5 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">Waktu Hadir</th>
                        <th class="px-10 py-5 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">Keterangan</th>
                        <th class="px-10 py-5 text-right text-[10px] font-black text-slate-400 uppercase tracking-widest">Aksi</th>
                    </tr>
                </thead>
                    ...
                    <tr class="group hover:bg-slate-50/30 transition-all">
                        <td class="px-10 py-6">
                            <div class="flex items-center gap-5">
                                <div class="w-12 h-12 rounded-2xl bg-slate-50 text-slate-400 flex items-center justify-center font-black text-xs border border-slate-100 group-hover:bg-white group-hover:shadow-md transition-all">
                                    {{ strtoupper(substr($attendance->patient->full_name ?? 'P', 0, 2)) }}
                                </div>
                                <div>
                                    <p class="font-black text-slate-900 text-sm leading-tight">{{ $attendance->patient->full_name ?? '-' }}</p>
                                    <p class="text-[11px] font-black text-slate-400 uppercase tracking-widest mt-1">RW: {{ $attendance->patient->rw ?? '—' }} / RT: {{ $attendance->patient->rt ?? '—' }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="px-10 py-6">
                            <div class="flex items-center gap-2 text-slate-700 font-bold text-sm">
                                ...
                                {{ $attendance->created_at->format('H:i') }} <span class="text-[10px] text-slate-400 font-black uppercase ml-1">WIB</span>
                            </div>
                        </td>
                        <td class="px-10 py-6">
                            <p class="text-sm font-medium text-slate-500 max-w-xs truncate">{{ $attendance->notes ?: 'Hadir secara fisik di lokasi kegiatan.' }}</p>
                        </td>
            ...
            <div class="px-10 py-32 text-center">
                <div class="max-w-xs mx-auto">
                    ...
                    <p class="text-sm text-slate-400 font-medium">Data akan terisi otomatis saat kader melakukan input pemeriksaan rekam medis hari ini.</p>
                </div>
            </div>
            @endif
        </div>
