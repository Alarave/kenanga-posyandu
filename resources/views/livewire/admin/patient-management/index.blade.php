<div class="space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 relative">
            <div class="space-y-2">
                ...
                <div class="max-w-2xl relative z-10">
                    <h1
                        class="font-display-sm md:font-display-lg text-display-sm-mobile md:text-display-lg text-teal-700 mb-2 tracking-tight">
                        Data Warga Terdaftar</h1>
                    <p class="font-body-lg text-body-lg text-on-surface-variant">Kelola data pasien posyandu di wilayah
                        Anda secara komprehensif.</p>
                </div>
            </div>
            ...
            <div class="flex flex-wrap gap-3 items-center ml-auto">
                @can('create', App\Models\Patient::class)
                    <a href="{{ route('admin.patients.import') }}"
                        ...
                        <span
                            class="material-symbols-outlined text-[20px] text-outline-variant group-hover/btn:text-teal-500 transition-colors">publish</span>
                        Import Data
                    </a>
    ...
    @if (session('import_errors') && count(session('import_errors')) > 0)
        <div
            class="p-6 bg-amber-50 border border-amber-200 rounded-2xl text-sm flex flex-col gap-3 shadow-sm animate-in slide-in-from-top-4 duration-300">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <div class="stat-card flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-headline-sm font-bold text-on-surface leading-none"
    ...
    <section class="section-card p-4">
        <div class="flex flex-wrap items-center gap-4">
            ...
            <div class="flex-1 min-w-70 relative group">
                <span
                    class="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-outline-variant group-focus-within:text-primary transition-colors pointer-events-none">search</span>
                ...
                @if ($search)
                    <button wire:click="$set('search', '')"
                        class="absolute right-4 top-1/2 -translate-y-1/2 text-outline-variant hover:text-red-500 transition-colors">
    ...
    <div class="section-card overflow-hidden">
        <x-table>
            <thead style="background:#f9fafb; border-bottom:1px solid rgba(0,0,0,0.06);">
                <tr>
                    <th class="px-5 py-3.5 text-xs font-semibold text-outline uppercase tracking-wider text-left">Nama
                        Warga</th>
                    <th class="px-5 py-3.5 text-xs font-semibold text-outline uppercase tracking-wider text-left">
                        Kategori</th>
                    <th class="px-5 py-3.5 text-xs font-semibold text-outline uppercase tracking-wider text-left">
                        Posyandu</th>
                    <th class="px-5 py-3.5 text-xs font-semibold text-outline uppercase tracking-wider text-right">Aksi
                    </th>
                </tr>
            </thead>
            <tbody class="divide-y divide-slate-50">
                @forelse($patients as $patient)
                    ...
                    <tr class="hover:bg-surface-container-low transition-colors"
                        style="border-bottom:1px solid rgba(0,0,0,0.04);" wire:key="patient-{{ $patient->id }}">
                        <td class="px-5 py-3.5">
                            <div class="flex items-center gap-3">
                                @if ($patient->profile_photo)
                                    <img src="{{ asset('storage/' . $patient->profile_photo) }}"
                                        class="h-9 w-9 rounded-xl object-cover border border-slate-100 shrink-0">
                                @else
                                    <div
                                        class="h-9 w-9 rounded-xl bg-primary-container text-on-primary-container flex items-center justify-center font-bold text-xs shrink-0">
                                        {{ $initials }}
                                    </div>
                                @endif
                                <div>
                                    ...
                                    <div class="text-xs text-outline-variant mt-0.5">NIK: {{ $patient->id_number }}
                                    </div>
                                </div>
                            </div>
                        </td>
                    ...
                    <tr>
                        <td colspan="4" class="px-6 py-24 text-center">
                            <div class="flex flex-col items-center gap-4 text-slate-300">
                                ...
                                <p class="text-sm font-bold text-outline uppercase tracking-widest">Tidak ada warga
                                    ditemukan</p>
                            </div>
                        </td>
                    </tr>
