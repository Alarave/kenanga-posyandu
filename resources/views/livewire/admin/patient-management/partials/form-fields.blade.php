        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <div class="lg:col-span-3 flex flex-col items-center">
                <div class="relative group">
                    <div class="rounded-[3rem] border-2 border-dashed border-slate-200 bg-slate-50/50 flex items-center justify-center overflow-hidden transition-all duration-500 group-hover:border-teal-500 group-hover:bg-teal-50/5 shadow-inner" style="width: 192px; height: 192px; shrink-0;">
                ...
                <div class="mt-8 text-center">
                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">{{ $isEdit ? 'Ubah Foto Profil' : 'Format Gambar' }}</p>
                    ...
                    @foreach(['SD', 'SMP', 'SMA', 'D3', 'S1', 'S2', 'S3', 'Tidak Sekolah'] as $edu)
                        <label class="cursor-pointer">
                            <input type="radio" name="education" value="{{ $edu }}" 
                                   ...
                                   class="hidden peer"
                                   x-bind:disabled="['bayi', 'baduta', 'balita', 'anak_sekolah'].includes(category)">
                            <div class="py-3 px-2 text-center border border-slate-200 rounded-2xl peer-checked:border-teal-600 peer-checked:bg-teal-50/50 peer-checked:text-teal-600 font-bold text-xs hover:border-slate-300 hover:bg-slate-50 transition-all">
                                {{ $edu }}
                            </div>
                        </label>
                    @endforeach
                ...
                <div class="flex flex-wrap gap-1.5 mt-2">
                    @foreach(['IRT', 'PNS', 'Karyawan', 'Wiraswasta', 'Buruh', 'Tidak Bekerja'] as $jobChip)
                        <button type="button" @click="currentJob = '{{ $jobChip }}'" 
                                class="px-2.5 py-1 rounded-lg border border-slate-200 hover:border-teal-500 hover:bg-teal-50/50 text-[10px] font-bold text-slate-500 hover:text-teal-600 transition-all">
                            {{ $jobChip }}
                        </button>
                    @endforeach
                </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div class="md:col-span-2 space-y-3">
                ...
                <textarea name="historical_diseases" rows="3" placeholder="Tuliskan riwayat penyakit seperti Hipertensi, Diabetes, Asam Urat..."
                          class="w-full p-6 border border-slate-200 rounded-4xl text-sm font-bold text-slate-700 focus:outline-none focus:border-teal-500 focus:ring-4 focus:ring-teal-500/5 transition-all bg-slate-50/30 resize-none"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div class="md:col-span-2 space-y-3">
                ...
                <textarea name="address" rows="3" required
                          ...
                          class="w-full p-6 border border-slate-200 rounded-4xl text-sm font-bold text-slate-700 focus:outline-none focus:border-teal-500 focus:ring-4 focus:ring-teal-500/5 transition-all bg-slate-50/30 resize-none">{{ old('address', $patient->address ?? '') }}</textarea>
            </div>
        </div>
    </div>
