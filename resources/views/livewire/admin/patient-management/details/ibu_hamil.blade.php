    ...
    <div class="px-12 pb-12 pt-10 space-y-10">
        ...
        <div>
            ...
            <div class="space-y-5">
                ...
                <div class="pb-4 border-b border-slate-100">
                    <p class="text-sm font-black text-outline-variant uppercase tracking-widest mb-2">Nama Suami</p>
                    <span class="text-body-lg font-black text-on-surface">{{ $patient->husband_name ?: $patient->parent_name ?: '-' }}</span>
                </div>
                {{-- Gender --}}
                <div class="pb-4 border-b border-slate-100">
        ...
        <div>
            ...
            <div class="space-y-5">
                ...
                {{-- Kelurahan --}}
                <div class="pb-4 border-b border-slate-100">
                    <p class="text-sm font-black text-outline-variant uppercase tracking-widest mb-2">Kelurahan / Desa</p>
                    <span class="text-body-lg font-black text-on-surface">{{ $patient->desa_kelurahan ?? '-' }}</span>
                </div>
                {{-- Kecamatan --}}
                <div class="pb-4 border-b border-slate-100">
                    <p class="text-sm font-black text-outline-variant uppercase tracking-widest mb-2">Kecamatan</p>
                    <span class="text-body-lg font-black text-on-surface">{{ $patient->kecamatan ?? '-' }}</span>
                </div>
                {{-- Alamat --}}
                <div class="pb-4 last:border-b-0 last:pb-0">
...
<div class="bg-white rounded-[3rem] border border-slate-100 p-10 shadow-[0_8px_30px_rgb(0,0,0,0.02)] mt-8">
    <div class="flex items-center gap-4 mb-8">
        ...
        <h4 class="text-base font-black text-on-surface uppercase tracking-widest">Pemeriksaan Kehamilan Terakhir</h4>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        {{-- Antropometri --}}
        <div class="p-8 rounded-2xl border border-slate-100 bg-surface-container-low/50">
            <p class="text-xs font-black text-outline-variant uppercase tracking-widest mb-4">Berat & Tinggi Badan</p>
            <div class="flex justify-between items-center py-2.5 border-b border-slate-100">
                <span class="text-sm font-semibold text-outline">Berat</span>
                <span class="text-base font-black text-on-surface">{{ $lastRecord->weight ?? '-' }} kg</span>
            </div>
            <div class="flex justify-between items-center py-2.5">
                <span class="text-sm font-semibold text-outline">Tinggi</span>
                <span class="text-base font-black text-on-surface">{{ $lastRecord->height ?? '-' }} cm</span>
            </div>
        </div>

        {{-- LILA (Lingkar Lengan Atas) --}}
        <div class="p-8 rounded-2xl border border-slate-100 bg-surface-container-low/50 flex flex-col justify-between">
            <div>
                <p class="text-xs font-black text-outline-variant uppercase tracking-widest mb-2">LILA (Lingkar Lengan Atas)</p>
                @if(isset($lastRecord->upper_arm_circumference))
                    <p class="text-display-sm text-on-surface">{{ $lastRecord->upper_arm_circumference }} <span class="text-sm font-bold text-outline-variant">cm</span></p>
                @else
                    <p class="text-display-sm text-outline-variant">—</p>
                @endif
            </div>
            
            @if(isset($lastRecord->upper_arm_circumference))
                @if($lastRecord->upper_arm_circumference < 23.5)
                    <div class="mt-4 px-4 py-2 rounded-xl bg-red-50 border border-red-100 text-error text-xs font-black uppercase tracking-wider flex items-center gap-1.5">
                        <span class="material-symbols-outlined text-[16px]">warning</span>
                        Risiko KEK
                    </div>
                @else
                    <div class="mt-4 px-4 py-2 rounded-xl bg-emerald-50 border border-emerald-100 text-primary text-xs font-black uppercase tracking-wider flex items-center gap-1.5 w-max">
        ...
        <div class="p-8 rounded-2xl border border-slate-100 bg-surface-container-low/50 flex flex-col justify-between">
            <div>
                <p class="text-xs font-black text-outline-variant uppercase tracking-widest mb-2">Suplementasi Tablet Fe</p>
                @if(isset($lastRecord->pill_fe))
                    <p class="text-headline-sm font-black text-on-surface">{{ $lastRecord->pill_fe ? 'Sudah Diberikan' : 'Belum Diberikan' }}</p>
                @else
                    <p class="text-display-sm text-outline-variant">—</p>
                @endif
            </div>
