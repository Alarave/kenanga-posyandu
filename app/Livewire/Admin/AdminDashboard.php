...
class AdminDashboard extends BaseAdminComponent
{
    ...
    public function updatedFilterPeriode() { $this->loadStatistics(); }
    public function updatedFilterCustomStartDate() { $this->loadStatistics(); }
    public function updatedFilterCustomEndDate() { $this->loadStatistics(); }
    public function updatedFilterPosyandu() { $this->loadStatistics(); }
    public function updatedFilterRisiko() { $this->loadStatistics(); }
    ...
    protected function getLansiaDemografi(Builder $patientQuery): array
    {
        ...
        foreach ($lansia as $l) {
            if ($l->birth_date) {
                ...
                if ($age >= 70) { $group70++; } elseif ($age >= 60) { $group60++; }
            }
        }
    ...
    protected function getMonthlyWeighingData(Builder $medicalRecordQuery): array {
        ...
        $trends = (clone $medicalRecordQuery)->when($this->filterPeriode === "semua", function($query) use ($startDate) { return $query->where("visit_date", ">=", $startDate); })->selectRaw("$dateFormat as month_year")->selectRaw("COUNT(*) as total")->groupBy("month_year")->get()->pluck("total", "month_year");
        $labels = []; $data = [];
        for ($i = 11; $i >= 0; $i--) {
