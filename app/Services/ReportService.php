...
class ReportService
{
    ...
    public function generateMonthlyReport(int $posyanduId, int $month, int $year): array
    {
        ...
        foreach ($records as $record) {
            ...
            if ($isFirstVisit) {
                if ($age <= 11) $pokjaIvData['cohorts']['0-11_baru'][$g]++;
                else $pokjaIvData['cohorts']['12-59_baru'][$g]++;
            }

            if ($age === 12) $pokjaIvData['cohorts']['12_bln'][$g]++;
            if ($age === 24) $pokjaIvData['cohorts']['24_bln'][$g]++;
            if ($age === 36) $pokjaIvData['cohorts']['36_bln'][$g]++;
            if ($age === 48) $pokjaIvData['cohorts']['48_bln'][$g]++;
            if ($age === 59) $pokjaIvData['cohorts']['60_bln'][$g]++; // Handling 60 as 59 edge

            if ($record->weight >= 11.5) $pokjaIvData['cohorts']['lulus'][$g]++;
        }
